...
def test_indicator_period_combinations(use_parallel: bool = True, max_workers: int | None = None) -> pd.DataFrame:
    ...
    """
    # Load data
    data = pd.read_csv('data/5m/SPY.csv')
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_combo):
                try:
                    ...
                    print(f"Completed backtest for roc_of_roc={result['description_of_indicator_periods']['roc_of_roc']}")
    ...
    if results:
        ...
        result_df.to_csv('results.csv', mode='w', index=False)
        print(f"Saved {len(results)} results to results.csv")    
    return pd.DataFrame(results) if results else pd.DataFrame()


def backtest(data: pd.DataFrame, indicator_periods: dict = {}, signal_thresholds: dict | None = None) -> dict:
    """
    ...
    for _, row in data.iterrows():
        if trade_open:
            if check_signal(row, 'sell', len(indicator_periods)-1, signal_thresholds) or row['close'] < entry_price * (1-stop_loss_pct) or row['close'] > entry_price * (1+trailing_stop_pct):
            ...
            if check_signal(row, 'buy', len(indicator_periods), signal_thresholds):
...
def check_signal(row: pd.Series, signal_type: str, conditions_to_be_met: int, signal_thresholds: dict) -> bool:
    """
    ...
    if 'volatility' in row:
        ...
        if (signal_type == 'buy' and row['volatility'] > threshold) or (signal_type == 'sell' and row['volatility'] < threshold):
            conditions_met += 1    # Price change check
    # Price change check
...
def buy(row: pd.Series, conditions_to_be_met: int = 0, signal_thresholds: dict | None = None) -> bool:
    """
    Buy signal - wrapper for check_signal function
    """
    if signal_thresholds is None:
        signal_thresholds = DEFAULT_SIGNAL_THRESHOLDS.copy()
    return check_signal(row, 'buy', conditions_to_be_met, signal_thresholds)
    
def sell(row: pd.Series, conditions_to_be_met: int = 0, signal_thresholds: dict | None = None) -> bool:
    """
    Sell signal - wrapper for check_signal function
    """
    if signal_thresholds is None:
        signal_thresholds = DEFAULT_SIGNAL_THRESHOLDS.copy()
    return check_signal(row, 'sell', conditions_to_be_met, signal_thresholds)
